...
public class Player_Interaction_No_Minigames : MonoBehaviour {
    ...
    [Header("-----SCRIPTS----")] 
    Cooking_No_Minigames cooktopScript;
    ...
    private void Update() {
        if(cooktopRange) {
            if(!cooktopScript.IsCooking() && PickupSystem.inst.isHoldingIngredient() && !cooktopScript.IsFoodReady()) {
                // use TakeAction function to display a prompt and await user interaction
                if (TakeAction("[C] Cook", KeyCode.C)) {
                    cooktopScript.StartCooking();
                    SetInteraction(false);
                }
            }
            // if food is ready
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if(collision.tag == "Cooktop") { //&& !cooktopScript.IsCooking()) {
            ...
            cooktopRange = true;
            cooktopScript = collision.gameObject.transform.GetComponent<Cooking_No_Minigames>();
        }
