...
public class Player_Interaction_Counters : MonoBehaviour {
            ...
            if(PickupSystem.inst.isHoldingSomething()) {
                if(!counterScript.full()) {
                    if (TakeAction("[F] Place Item", KeyCode.F)) {
                        counterScript.set_item(PickupSystem.inst.playerHolding);
                        PickupSystem.inst.DropItem();
                        SetInteraction(false);
                    }
                } else {
                    Prompt("Counter is Full");
                }
            }
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if (collision.tag == "Cooktop") { //&& !cooktopScript.IsCooking()) {
            ...
            counterRange = true;
            counterScript = collision.gameObject.transform.GetComponent<Counter>();
